...
const { newTodoText, newTodoContent, filter, showDocuments, editable }
...
const contentAnalysis = computed(() => {
  ...
  return {
    ...
    characterCount:
      editor_content.value.storage.characterCount?.characters?.() || 0,
  }
})
  ...
  <div class="font-mono text-foreground">
    ...
    >
      ...
      >
        ...
        <div
          :class="
            sidebar_splitter_ref?.isCollapsed
              ? 'fixed bottom-0 left-0'
              : 'fixed z-[80] left-0 bottom-0 m-px'
          "
        >
          <DialogCommandMenu />
        </div>
        ...
        >
          <div class="w-full @container pt-8">
            ...
            >
              <div
                class="w-full flex justify-center mt-1 @xs:justify-start items-center gap-2 font-bold"
              >
              ...
              >
                <div class="view-mode w-full flex gap-1 flex-col sm:flex-row">
                  <button
                    class="w-full px-3"
                    :class="
                      todo.id === editable_id ? 'text-primary font-bold' : ''
                    "
                    @click="editTodo(todo.id)"
                  >
                    <span
                      class="line-clamp-1 w-full text-left text-sm overflow-y-hidden"
                    >
                      {{ todo.text.length === 0 ? "---" : todo.text }}
                    </span>
                  </button>
                  <div
                    class="flex gap-1 items-center bg-secondary gap-x-px pl-px justify-end"
                  >
                    <button
